...
- [功能清单](#-功能清单)
  - [账号监控](#1-账号监控)
  - [Git 远程仓库监控](#2-git-远程仓库监控)
  - [MCP / Skill 扫描](#3-mcp--skill-扫描)
  - [敏感信息检测](#4-敏感信息检测)
  - [防绕过机制](#5-防绕过机制)
  - [仪表盘](#6-仪表盘)
- [安装](#-安装)
...
| 监控命令                          | 告警描述             |
| ----------------------------- | ---------------- |
| `git push <非origin>`          | 推送到非 origin 远程仓库 |
| `git push --all` / `--mirror` | 推送所有分支/镜像        |
| `git remote add`              | 添加新的远程仓库         |
| `git remote set-url`          | 修改远程仓库地址         |
| `git remote rm`               | 删除远程仓库配置         |
| `git config --global`         | 修改全局 Git 配置      |
| `git credential`              | 操作 Git 凭证        |
| `export *KEY=*TOKEN=`         | 设置敏感环境变量         |
| `curl -H Authorization`       | HTTP 请求含认证头      |
| `ssh-keygen`                  | 生成 SSH 密钥        |
| `aws * --secret`              | AWS CLI 使用密钥参数   |
...
| 防护层级  | 实现方式                                | 触发行为                                                      |
| ----- | ----------------------------------- | --------------------------------------------------------- |
| 禁用监控  | `onDidChange` 监听自身扩展状态              | 弹模态框 + 自动 `activate()` 恢复                                 |
| 卸载监控  | 启动时校验核心文件完整性                        | 文件缺失弹紧急告警                                                 |
| 文件完整性 | `fs.watch` 递归监听扩展目录                 | 核心文件删除弹紧急告警                                               |
| 配置防篡改 | `onDidChangeConfiguration` + 可信配置快照 | 自动恢复 `enabled`、`allowedEmailDomains`、`allowedRepos` 等关键配置 |

#### 配置自动恢复策略

| 被篡改的配置                         | 恢复行为         |
| ------------------------------ | ------------ |
| `enabled` → `false`            | 自动恢复为 `true` |
| `allowedEmailDomains` → `[]`   | 恢复上次快照       |
| `allowedRepos` → `[]`          | 恢复上次快照       |
| `blockCommitOnLeak` → `false`  | 恢复为 `true`   |
| `autoRecoverEnabled` → `false` | 恢复为 `true`   |

#### 心跳上报

| 配置      | 说明                                       |
| ------- | ---------------------------------------- |
| 端点      | `cursorSecurity.heartbeatEndpoint`（空=禁用） |
| 间隔      | 5 分钟                                     |
| 协议      | HTTP/HTTPS POST                          |
| 超时      | 10 秒                                     |
| Payload | 扩展状态、账号、仓库、扫描统计                          |

### 6. 仪表盘

侧边栏 Webview 面板，包含 6 张卡片：
...
| `git remote set-url` 修改远程地址 | ✅      | 远程 URL 变更检测                     |
| 推送到未授权仓库（UI）                | ✅      | 推送前全远程校验                        |
| 推送含密钥的文件                    | ✅      | 提交前拦截 + 暂存前拦截                   |
| 编辑器中写入密钥                    | ✅      | 实时装饰器（红色波浪线）                    |
| 复制密钥到剪贴板                    | ✅      | 剪贴板监控（2s 轮询）                    |
| 禁用安全插件                      | ✅      | 自动恢复 + 告警                       |
| 修改安全配置                      | ✅      | 配置防篡改 + 自动恢复                    |
| `git push other-remote`（终端） | ⚠️ 间接  | 远程变更检测（30s 后感知）                 |
| 终端 `export API_KEY=xxx`     | ⚠️ 待激活 | 终端监控（VS Code API 限制）            |
