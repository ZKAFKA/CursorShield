...
function checkRepoCompliance(url, allowedRepos) {
    if (!url) {
        return false;
    }
...
function checkAllRemotesCompliance(remotes, allowedRepos) {
    if (allowedRepos.length === 0) {
        return [];
    }
    const violations = [];
        ...
        for (const [name, url] of currentRemotes) {
            ...
            if (oldUrl === undefined) {
                ...
                if (!isCompliant && allowedRepos.length > 0) {
                    vscode.window.showWarningMessage(`[Cursor Shield] 检测到新增未授权远程仓库：${name} (${extractRepoName(url)})`, '我知道了');
                }
            }
            else if (oldUrl !== url) {
                ...
                const isCompliant = checkRepoCompliance(url, allowedRepos);
                if (!isCompliant && allowedRepos.length > 0) {
                    vscode.window.showWarningMessage(`[Cursor Shield] 检测到远程仓库地址变更为未授权仓库：${name} (${extractRepoName(url)})`, '我知道了');
                }
            }
        }
...
function registerGitMonitor(context) {
    const disposables = [];
    createStatusBarItem();
    ...
    const pushCheckCommand = vscode.commands.registerCommand('cursorSecurity.checkBeforePush', async () => {
        const config = vscode.workspace.getConfiguration('cursorSecurity');
        const blockPush = config.get('blockPushOnLeak', true);
        if (!blockPush) {
            return true;
        }
        await initGitApi();
        ...
        if (violations.length > 0) {
            vscode.window.showErrorMessage(`[Cursor Shield] 检测到未授权远程仓库，禁止推送：\n${violations.join('\n')}`, { modal: true }, '我知道了');
    ...
    const refreshCommand = vscode.commands.registerCommand('cursorSecurity._refreshGit', () => {
        apiReady = false;
        gitApi = null;
        checkGitRepo();
    });
...
//# sourceMappingURL=gitRemote.js.map